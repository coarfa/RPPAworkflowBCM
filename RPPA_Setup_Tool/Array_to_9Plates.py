...
# =============================================================================
# Discription: This is the 9 plate version
...
def main(input_file):
    ...
    for l in reversed(range(3)):
        for k in range(6):
            ...
            for m in range(10):
                list2 = []
                for n in range(4):
                    list2.append(DF.iloc[6*m + k, 3*n + l]) # this is kth row and the lth column on every block
                #print list
    ...
    #            offset = offset.append(slide1, ignore_index=True)
                slide1.to_csv('RPPA_CSV_p9 P{}.csv'.format(slide_no), header=False, index=False)
    #            offset.to_csv('RPPA_CSV.v6 P{}.csv'.format(slide_no), header=False, index=False)
                slide_no += 1
    DF.to_csv('test_array9.csv', header=False, index=False)
