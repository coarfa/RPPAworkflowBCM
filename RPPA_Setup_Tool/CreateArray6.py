...
# =============================================================================
# Discription: Create Array. Assigns the samples. This is the 6 plate version
...
def fill_empty(index): 
    fill = pd.DataFrame(np.full((12*PI_spots.iloc[index]['blocks'] - PI_spots.iloc[index]['spots'],2), 'Empty'))
    return fill
...
def plateInit():
    ...
    for r1 in range(10):
        for c1 in range(4):
            Plate[r1,c1] = np.zeros([4, 3], dtype='|S32') 
    return Plate

def SaveMe(df):
    writer = pd.ExcelWriter('output_sample_p6_assignment.xlsx', engine='xlsxwriter')
...
def main(input_file):
    ...
    Plate[0,:] = [np.full((4,3), 'Empty', dtype='|S32'), np.full((4,3), 'Empty', dtype='|S32'),np.full((4,3), 'Empty', dtype='|S32'), np.full((4,3), 'Empty', dtype='|S32')]
    Plate[9,:] = [np.full((4,3), 'Empty', dtype='|S32'), np.full((4,3), 'Empty', dtype='|S32'), np.full((4,3), 'Empty', dtype='|S32'), np.full((4,3), 'Empty', dtype='|S32')]  
    
    IgGs = [x for x in pd.Series(CTRLS.count().index.tolist()) if 'IgGmix' in x]
    Plate[0,0][0,0] = 'Ctrl_' + IgGs[0] 
    Plate[0,3][0,2] = 'Ctrl_' + IgGs[1]  
    Plate[9,0][3,0] = 'Ctrl_' + IgGs[2] 
    Plate[9,3][3,2] = 'Ctrl_' + IgGs[3]
