...
# =============================================================================
# Discription: Create Array. Assigns the samples. This is the 12 plate version
...
def fill_empty(index):
    fill = pd.DataFrame(np.full((24*PI_spots.iloc[index]['blocks'] - PI_spots.iloc[index]['spots'],2), 'Empty'))
    return fill
...
def plateInit():
    ...
    for r1 in range(10):
        for c1 in range(4):
            Plate[r1,c1] = np.zeros([8, 3], dtype='|S32')
    return Plate
...
def main(input_file):            
    ...
    Plate[0,:] = [np.full((8,3), 'Empty', dtype='|S32'), np.full((8,3), 'Empty', dtype='|S32'),np.full((8,3), 'Empty', dtype='|S32'), np.full((8,3), 'Empty', dtype='|S32')]
    Plate[9,:] = [np.full((8,3), 'Empty', dtype='|S32'), np.full((8,3), 'Empty', dtype='|S32'), np.full((8,3), 'Empty', dtype='|S32'), np.full((8,3), 'Empty', dtype='|S32')]  
    
    IgGs = [x for x in pd.Series(CTRLS.count().index.tolist()) if 'IgGmix' in x]
    Plate[0,0][0,0] = 'Ctrl_' + IgGs[0] 
    Plate[0,3][0,2] = 'Ctrl_' + IgGs[1]  
    Plate[9,0][7,0] = 'Ctrl_' + IgGs[2] 
    Plate[9,3][7,2] = 'Ctrl_' + IgGs[3]
    ...
    for c in range(2): 
        ...
        if c==0:
            ...
            assign2([9,2,5,0], std, Plate)

        else:
            assign2([9,1,0,0], Ctrl, Plate)
    ...
    PI_spots['blocks'] = (PI_spots['spots']/24).apply(np.ceil).astype(int)
    PI_spots['blocks'].sum() 
    
    if len(PI_spots[PI_spots['blocks']==2])>=2: 
        
        p1 = str(PI_spots[PI_spots['blocks']==2].iloc[0]['index'])
        p2 = str(PI_spots[PI_spots['blocks']==2].iloc[1]['index']) 
